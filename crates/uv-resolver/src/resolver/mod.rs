...
impl<InstalledPackages: InstalledPackagesProvider> ResolverState<InstalledPackages> {
    ...
    fn solve(
        ...
        let mut forked_states = self.env.forked_states(state);
    ...
    fn forks_to_fork_states<'a>(
        ...
        forks
            ...
            .map(move |(i, fork)| {
                ...
                let mut forked_state = forked_state.with_env(&fork.markers);
        ...
        'data: 'parameters,
    {
        self.overrides
            ...
            .filter_map(move |requirement| {
                ...
                if !env.in_fork(&requirement.marker) {
                ...
                iter::once(requirement.clone()).chain(
                    self.constraints
                        ...
                        .filter_map(move |constraint| {
                            ...
                            // If we're in a fork in universal mode, ignore any dependency that isn't part of
                            // this fork (but will be part of another fork).
                            if !env.in_fork(&constraint.marker) {
...
impl ForkState {
    ...
    /// accordingly.
    fn with_env(mut self, markers: &MarkerTree) -> Self {
        self.env = self.env.narrow_markers(markers);
